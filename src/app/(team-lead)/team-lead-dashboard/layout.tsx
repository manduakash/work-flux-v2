            ...
            <aside className={cn(
                ...
                <div className="flex h-full flex-col px-4 py-6">
                    <div className="mb-10 flex items-center gap-3 px-2">
                        ...
                        {!isCollapsed && <span className="text-xl font-black tracking-tighter uppercase dark:text-white">LeadPanel</span>}
                    </div>
                    ...
                    <nav className="flex-1 space-y-2 overflow-y-auto">
                        {leadNavItems.map((item) => (
                            ...
                            >
                                <item.icon size={20} className={cn("shrink-0", pathname === item.href ? "text-emerald-600" : "group-hover:text-slate-900 dark:group-hover:text-slate-200")} />
                ...
                <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white/80 px-8 backdrop-blur-md dark:border-slate-800 dark:bg-slate-950/80">
                    <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="relative max-w-md w-full hidden md:block group">
                            ...
                            <input placeholder="Search resources, devs, or tickets..." className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 text-xs font-bold outline-none focus:ring-4 focus:ring-emerald-500/10 dark:border-slate-800 dark:bg-slate-900" />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3 rounded-xl border border-slate-200 p-1 pl-3 dark:border-slate-800">
                            <div className="text-right hidden sm:block">
                                <p className="text-[10px] font-black uppercase text-emerald-600 leading-none">Team Lead</p>
                                <p className="text-xs font-bold dark:text-white">Alex Rivera</p>
                            </div>
                            <div className="h-8 w-8 rounded-lg bg-emerald-600 flex items-center justify-center font-black text-white text-xs text-center">AR</div>
                        </div>
                    </div>
                </header>
